...
private func getQueue(queue queueType: DispatchQueue = .main) -> Dispatch.DispatchQueue {
  ...
  case .interactive:
    queue = Dispatch.DispatchQueue.global(qos: DispatchQoS.QoSClass.userInteractive)
  case .initiated:
    queue = Dispatch.DispatchQueue.global(qos: DispatchQoS.QoSClass.userInitiated)
  case .utility:
    queue = Dispatch.DispatchQueue.global(qos: DispatchQoS.QoSClass.utility)
  case .background:
    queue = Dispatch.DispatchQueue.global(qos: DispatchQoS.QoSClass.background)
